...
const tJam = _.clone(config.jam).reverse();
const tCompo = _.clone(config.compo).reverse();
...
export const sendAuthor = (req, res) => {
  ...
  req.author.entries = _.map(req.entries, entry => {
    ...
    _entry.totalCompo = tCompo[_entry.ludum-1];
    _entry.totalJam = tJam[_entry.ludum-1];
    _entry.total = _entry.totalCompo + _entry.totalJam;
